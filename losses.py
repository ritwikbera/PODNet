...
def DynamicsLoss(next_state_segment, next_state_pred, PAD_TOKEN):
    mask = (next_state_segment!=PAD_TOKEN).type(torch.FloatTensor)
    L_ODC = (((next_state_segment - next_state_pred)**2)*mask).sum(-1).sum(-2).mean()
    return L_ODC

def BCLoss(action_segment, action_pred, PAD_TOKEN, use_discrete=False):
    mask = (action_segment!=PAD_TOKEN).type(torch.FloatTensor)
    if use_discrete:
